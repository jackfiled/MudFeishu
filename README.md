...
graph TD
    ...
    subgraph CoreAPI["Mud.Feishu<br/>(HTTP API 核心)"]
        API1["用户管理"]
        API2["部门管理"]
        API3["消息服务"]
        API4["审批流程"]
    ...
    subgraph Redis["Mud.Feishu.Redis<br/>(分布式扩展)"]
        R1["事件去重"]
        R2["Nonce防重放"]
        R3["SeqID去重"]
    end
    
    subgraph Abs["Mud.Feishu.Abstractions<br/>(事件处理抽象层)"]
        Abs1["IFeishuEventHandler 接口"]
        Abs2["事件处理器工厂"]
        Abs3["策略模式架构"]
        Abs4["基类处理器"]
    end
    
    subgraph WH["Mud.Feishu.Webhook<br/>(HTTP 回调事件处理)"]
        WH1["中间件模式"]
        WH2["事件订阅验证"]
        WH3["AES-256-CBC 解密"]
        WH4["签名验证"]
...
- 用户事件：`contact.user.*_v3`
- 部门事件：`contact.department.*_v3`
- 审批事件：`approval.approval.*_v1`
...
| 功能分类 | 特性 |
|---------|------|
| **安全验证** | 签名验证、时间戳验证、AES-256-CBC 解密、IP 白名单 |
| **事件处理** | 中间件模式、自动路由、策略模式、异步处理 |
| **高级功能** | 多机器人支持、后台处理、并发控制、配置热更新 |
| **监控运维** | 性能监控、健康检查、请求日志、异常处理 |
| **安全加固** | 滑动窗口限流、威胁检测、安全审计、密钥验证 |

### 💾 Mud.Feishu.Redis - 分布式去重扩展

**基于 Redis 的分布式事件去重，适用于多实例部署**

| 功能分类 | 特性 |
|---------|------|
| **去重机制** | EventId、Nonce、SeqID 三种去重维度 |
| **原子操作** | SETNX + EXPIRE 确保原子性 |
| **自动过期** | 自动清理过期数据 |
| **分布式支持** | 集群模式、哨兵模式、TLS/SSL |
| **灵活配置** | 可配置过期时间、键前缀、超时 |
| **监控诊断** | 日志记录、缓存统计、健康检查 |
