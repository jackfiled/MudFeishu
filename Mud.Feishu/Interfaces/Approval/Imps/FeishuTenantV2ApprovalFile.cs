...
partial class FeishuTenantV2ApprovalFile
{
      ...
      CancellationToken cancellationToken = default)
    {
        ...
        var url = $"/approval/openapi/v2/file/upload";
        using var request = new HttpRequestMessage(HttpMethod.Post, url);

        request.Headers.Add("Authorization", access_token);

        ExceptionUtils.ThrowIfNull(uploadFileRequest, nameof(uploadFileRequest));
        ExceptionUtils.ThrowIfNullOrEmpty(uploadFileRequest.Type, nameof(uploadFileRequest.Type));
        ExceptionUtils.ThrowIfNullOrEmpty(uploadFileRequest.FullName, nameof(uploadFileRequest.FullName));
        ExceptionUtils.ThrowIfNullOrEmpty(uploadFileRequest.Name, nameof(uploadFileRequest.Name));

        using var formData = new MultipartFormDataContent
        {
            { new StringContent(uploadFileRequest.Type), "type" },
            { new StringContent(uploadFileRequest.Name), "name" }
        };
