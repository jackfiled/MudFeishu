...
public class App1SpecificInterceptor : IFeishuEventInterceptor
{
    ...
    {
        _logger.LogInformation("[App1拦截器] 事件开始处理: EventType={EventType}, EventId={EventId}, TenantKey={TenantKey}",
    ...
    public Task AfterHandleAsync(string eventType, EventData eventData, Exception? exception, CancellationToken cancellationToken = default)
    {
        if (exception != null)
        {
            _logger.LogError(exception, "[App1拦截器] 事件处理失败: EventType={EventType}, EventId={EventId}",
                eventType, eventData.EventId);
        }
        else
        {
            _logger.LogInformation("[App1拦截器] 事件处理成功: EventType={EventType}, EventId={EventId}",
                eventType, eventData.EventId);
        }
